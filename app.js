  ...
  cont.innerHTML = `
    ...
    <div class="bg-gradient-to-br from-purple-900/30 to-slate-800/80 rounded-2xl p-4 border border-purple-500/30">
      ...
      <p class="text-xs text-slate-400 leading-relaxed mb-3">
        Let Claude write a personalised summary of your investing progress (for learning — not buy/sell advice).
      </p>
      ${IS_LOCAL
        ? `<p class="text-xs text-amber-400/80">⚠️ Available only when deployed on Cloudflare (requires ANTHROPIC_API_KEY).</p>`
        : `<button id="ai-coach-btn" onclick="requestAICoach()"
             ...
             🤖 Ask Claude to analyse this month
           </button>`}
...
async function requestAICoach() {
  ...
  const m = calcPortfolio();
  if (!m.holdings.length) {
    out.innerHTML = '<p class="text-xs text-amber-400">No holdings yet — add a stock first for Claude to analyse.</p>';
    return;
  }

  if (btn) { btn.disabled = true; btn.textContent = '⏳ Claude is analysing…'; }
  out.innerHTML = '<p class="text-xs text-slate-400">Sending your portfolio to Claude…</p>';
  ...
  try {
    ...
    if (btn) { btn.disabled = false; btn.textContent = '🔄 Ask Claude again'; }
  }
}
  ...
  out.innerHTML = `
    <div class="space-y-3 bg-slate-900/50 rounded-xl p-3 border border-purple-500/20">
      ...
      ${(s.whatWentWell || []).length ? `
        <div>
          <p class="text-xs font-semibold text-emerald-400 mb-1">✅ What's Working Well</p>
          <ul class="space-y-1">${list(s.whatWentWell, 'text-emerald-500')}</ul>
        </div>` : ''}
      ${(s.whatToReview || []).length ? `
        <div>
          <p class="text-xs font-semibold text-amber-400 mb-1">🔍 What to Review</p>
          <ul class="space-y-1">${list(s.whatToReview, 'text-amber-500')}</ul>
        </div>` : ''}
      ${(s.questions || []).length ? `
        <div>
          <p class="text-xs font-semibold text-purple-400 mb-1">💭 Questions to Reflect On</p>
          <ul class="space-y-1">${list(s.questions, 'text-purple-400')}</ul>
        </div>` : ''}
      <p class="text-[10px] text-slate-500 pt-1">Generated by Claude · For learning only, not investment advice.</p>
    </div>`;
}
